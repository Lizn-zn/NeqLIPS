...
class Prover:
    ...
    def get_state(self, proof_state: int, tactics: List[str]) -> List[int]:
        ...
        for tactic in tactics:
            ...
            if new_proof_state == proof_state:
                ...
                continue
            try:
                goal_probe, new_proof_state = self.check_state(new_proof_state)
            except TimeoutError:
                goals = self.leanio.get_goals(proof_state)
                goal_probe = 0
                if self.prover_vb:
            ...
            if goal_probe == -1:  # illegal goals
                ...
                if self.prover_vb:
                    print(f"{tactic} failed because it has illegal goals")
                continue
            elif goal_probe == 1:
                print("FINISH!!!")
    ...
    @timeout(10)
    def check_state(self, proof_state: int) -> int:
        """
        -1: if the inequality does not contain >=, then it is illegal
        ...
        #### if no variable involved in the goal
        if utils.is_const(str(expr)):
            ...
            if len(goals) == 0:
                return 1, next_proof_state
            else:
                if next_proof_state != proof_state:
                    self.leanio.remove_step(next_proof_state)
        if utils.cal_depth(expr) <= 3 or utils.cal_calculating(expr, [1, 1, 1, 1, 1]) <= 8:
